...
class QrmDQN(nn.Module):
    def __init__(self, action_space, num_policies, env_name, device):
        super(QrmDQN, self).__init__()
        self.device = device
        ...
        if self.env_name == "two_keys":
            ...
            self.fc1 = nn.Linear(10 * 10 * 64 + num_policies, 512)
    ...
    def forward(self, x, code, x_extra=None, actions=None):
        if self.env_name == "two_keys":
            if x.dim() <= 3:
                x = x.unsqueeze(0)
            batch_size = x.size(0)
            ...
            x = x.view(batch_size, -1)
            x = torch.cat([x, code, x_extra, actions], dim=1)
            x = self.relu(self.fc1(x))
            x = self.fc2(x)
